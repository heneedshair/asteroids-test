...
// EN mirror of build-pdf-ru.cjs. Differences: reads pdf-manifest-en.json and
// renders the cover in English ("Version" / "Authors"). The RU build is left
// untouched. See scripts/build-pdf-ru.cjs for the original commentary.
const manifest = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'pdf-manifest-en.json'), 'utf-8')
);
...
// Strip a single leading YAML frontmatter block so it does not render as body
// text (md-to-pdf only parses frontmatter on the FIRST doc of a concatenated
// stream). We concatenate body-only here.
...
${doc.title} · Version ${manifest.version} | ${manifest.date}
