...
Реализация: [scripts/brain_universality_semantic.py](../../scripts/brain_universality_semantic.py). Pure stdlib; переиспользует FTS5 инфраструктуру [scripts/brain_search.py](../../scripts/brain_search.py). Никогда не выбрасывает исключений, не блокирует. Пустой mirror → silent no-op. Точность синонимов растёт по мере промоутинга записей в brain.
